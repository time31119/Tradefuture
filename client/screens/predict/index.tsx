...
export default function PredictScreen() {
  ...
  const fetchPriceHistory = useCallback(async (interval?: string) => {
    try {
      ...
      const data = await res.json();
      // API 返回格式: { success: true, data: { prices: [...] } }
      const prices = data.data?.prices || data.data || [];
      if (prices.length > 0) {
        const chartData = prices.map((p: { time: number; price: number }) => ({
          value: p.price,
        }));
        ...
        setChartHigh(Math.max(...prices.map((p: { high?: number; price: number }) => p.high || p.price)));
        setChartLow(Math.min(...prices.map((p: { low?: number; price: number }) => p.low || p.price)));
      }
    } catch (error) {
